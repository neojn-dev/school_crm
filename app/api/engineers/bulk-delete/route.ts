...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!deleteAll && (!ids || !Array.isArray(ids) || ids.length === 0)) {
      return NextResponse.json({ 
        error: 'Engineer IDs are required for bulk delete' 
      }, { status: 400 })
    }
    ...
    if (deleteAll) {
      ...
      const result = await prisma.engineer.deleteMany({})
    ...
    return NextResponse.json({
      success: true,
      message: `Successfully deleted ${deletedCount} engineer${deletedCount !== 1 ? 's' : ''}`,
      deletedCount
    })

  } catch (error) {
    console.error('Bulk delete engineers error:', error)
    return NextResponse.json(
