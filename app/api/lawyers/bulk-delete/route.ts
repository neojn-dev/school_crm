...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!deleteAll && (!ids || !Array.isArray(ids) || ids.length === 0)) {
      return NextResponse.json({ 
        error: 'Lawyer IDs are required for bulk delete' 
      }, { status: 400 })
    }
    ...
    if (deleteAll) {
      // Delete all lawyers
      const result = await prisma.lawyer.deleteMany({})
      deletedCount = result.count
    } else {
      // Delete selected lawyers
      const result = await prisma.lawyer.deleteMany({
        where: {
          id: { in: ids }
        }
    ...
    return NextResponse.json({
      success: true,
      message: `Successfully deleted ${deletedCount} lawyer${deletedCount !== 1 ? 's' : ''}`,
      deletedCount
    })

  } catch (error) {
    console.error('Bulk delete lawyers error:', error)
    return NextResponse.json(
