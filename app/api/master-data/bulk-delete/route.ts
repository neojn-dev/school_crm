...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!deleteAll && (!ids || !Array.isArray(ids) || ids.length === 0)) {
      return NextResponse.json({ 
        error: 'Master Data IDs are required for bulk delete' 
      }, { status: 400 })
    }
    ...
    if (deleteAll) {
      ...
      const result = await prisma.masterData.deleteMany({})
      deletedCount = result.count
    } else {
      // Delete selected master data
      const result = await prisma.masterData.deleteMany({
        where: {
          id: { in: ids }
        }
    ...
    return NextResponse.json({
      success: true,
      message: `Successfully deleted ${deletedCount} master data record${deletedCount !== 1 ? 's' : ''}`,
      deletedCount
    })

  } catch (error) {
    console.error('Bulk delete master data error:', error)
    return NextResponse.json(
